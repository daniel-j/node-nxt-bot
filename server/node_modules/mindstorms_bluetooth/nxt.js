...
var Nxt = function (port) {
	...
	//Sensor types
	this.NO_SENSOR = 0x00;
	this.SWITCH = 0x01;		// Touch sensor
	this.TEMPERATURE = 0x02;
	this.REFLECTION = 0x03;
	this.ANGLE = 0x04;
	this.LIGHT_ACTIVE = 0x05;	// Light sensor (with LED)
	this.LIGHT_INACTIVE = 0x06;	// Light sensor (ambient)
	this.SOUND_DB = 0x07;		// Sound sensor
	this.SOUND_DBA = 0x08;		// Sound sensor (human hearable sounds)
	this.CUSTOM = 0x09;
	this.LOWSPEED = 0x0a;
	this.LOWSPEED_9V = 0x0b;	// Ultrasonic sensor (I2C, lowspeed)
	this.NO_OF_SENSOR_TYPES = 0x0c;
	this.COLOR_FULL = 0x0d;
	this.COLOR_RED = 0x0e;
	this.COLOR_GREEN = 0x0f;
	this.COLOR_BLUE = 0x10;
	this.COLOR_NONE = 0x11;
	this.COLOR_EXIT = 0x12;
...
Nxt.prototype.start_program = function (program_name) {
	...
	var i;
	for (i in chars) {
		command_arr.push(chars[i].charCodeAt(0));
	}
	command_arr.push(0x00);
...
Nxt.prototype.message_write = function (inbox_no, message) {
	var command = new Buffer(5+message.length);
	command[0] = 0x00;
	command[1] = 0x09;
	command[2] = inbox_no;
...
Nxt.prototype.message_read = function (inbox_no) {
	var command = new Buffer([0x00, 0x13, inbox_no+10, 0x00, 0x01]);
	this.execute_command(command);
};
...
Nxt.prototype.status_handle = function (data) {
	data = data.slice(2);
	if(data[2] === 0x40) return;
	//console.log(data);
	if (data[2] > 0) {
		console.log("NXT error: " + this.nxt_error_messages[data[2]]);
	} else {
